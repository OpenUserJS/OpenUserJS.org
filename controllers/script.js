...
/*exports.useLib = function (aReq, aRes, aNext) {
  var installName = aReq.route.params.shift() + '/'
    + aReq.route.params.shift();
  var user = aReq.session.user;
  var options = { username: user ? user.name : '' };

  Script.findOne({ installName: installName + '.js' }, function (aErr, aLib) {
    if (aErr || !aLib) { return aNext(); }

    options.title = 'Scripts that use <a href="/libs/' + installName + '">'
      + aLib.name + '</a>';
    modelsList.listScripts({ uses: aLib.installName },
      aReq.route.params, '/use/lib/' + installName,
      function (aScriptsList) {
        options.scriptsList = aScriptsList;
        aRes.render('group', options);
      });
  });
};*/
...
exports.edit = function (aReq, aRes, aNext) {
  ...
  Script.findOne({
    ...
    if (aReq.body.remove) {
      ...
      // Group.find({ _scriptIds: script._id }, 'name', function (aErr, aGroups) {
      //   var groupsArr = (aGroups || []).map(function (aGroup) {
      //     return aGroup.name;
      //   });

      //   aRes.render('scriptEdit', {
      //     title: script.name,
      //     name: script.name,
